...
local function getFarmBounds()
    local farm = workspace.Farm:FindFirstChild("Farm1")
...
DeliveryTab:CreateButton({
    ...
    callback = function()
        ...
        for item,cnt in pairs(inv) do
            if DeliveryConfig.Items[item] and cnt>0 then
...
-- MAGNET TAB
MagnetTab:CreateText({ name = "Auto Ambil", text = "Ambil produk di sekitar peternakan" })
local magnetToggle = MagnetTab:CreateToggle({
    name = "Auto Ambil",
    description = "Otomatis mengambil produk di sekitar peternakan",
...
local jarakSlider = MagnetTab:CreateSlider({
    name = "Jarak Ambil",
    range = {8,40}, increment = 2, value = 25, suffix = " studs", flag = "JarakAmbil",
    callback = function(v) cfg.jarakAmbil = v; notify("Jarak Ambil", v.." studs", 1.5) end,
})
MagnetTab:CreateButton({
    name = "AMBIL SEKARANG",
    description = "Ambil 5 produk terdekat",
    callback = function()
        notify("Ambil", "Mengambil 5...", 1.5)
        local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        local farm = workspace.Farm:FindFirstChild("Farm1")
        if not hrp or not farm then notify("Gagal", "Farm tidak ditemukan", 2) return end
        ...
        for _, tool in ipairs(farm:GetChildren()) do
            if tool:IsA("Tool") then
                ...
                local should = cfg.itemDiambil=="Semua" or (cfg.itemDiambil=="Emas & Cosmic" and (tool.Name:find("Emas") or tool.Name:find("Cosmic"))) or (cfg.itemDiambil=="Sakura & Cosmic" and (tool.Name:find("Sakura") or tool.Name:find("Cosmic"))) or (cfg.itemDiambil=="Emas" and tool.Name:find("Emas")) or (cfg.itemDiambil=="Cosmic" and tool.Name:find("Cosmic"))
                if should and prompt and prompt.Enabled then
                    pcall(function() hrp.CFrame = handle.CFrame * CFrame.new(0,3,0) end)
                    task.wait(0.35)
        ...
        log("Ambil "..collected.." delta "..delta)
        if delta>0 then notify("Berhasil", "+"..delta.." item ke tas!", 2.5) else notify("Gagal", "Tas penuh?", 2.5) end
    end,
})
...
task.spawn(function()
    while alive() do
        ...
        if cfg.autoKirim and alive() and not LocalPlayer:GetAttribute("IsDelivering") then
            ...
            elseif cfg.kirimBerdasarkan == "Saat tas penuh" then local t=getTotalItems(); if t>=350 then shouldSend=true; reason="tas "..t.."/360" end
            elseif cfg.kirimBerdasarkan == "Saat berat penuh" then local w=getTotalWeight(); local lvl=LocalPlayer:GetAttribute("DeliveryLevel") or 1; local cap=(DeliveryConfig.Levels[lvl] and DeliveryConfig.Levels[lvl].Capacity) or 4000; if w>=cap-100 then shouldSend=true; reason="berat "..w.."/"..cap end
            elseif cfg.kirimBerdasarkan == "Saat ada item" then if getTotalItems()>20 then shouldSend=true; reason="ada item "..getTotalItems() end end
            if shouldSend then
                local inv=getInventory()
                local payload, totalW={},0
                local lvl=LocalPlayer:GetAttribute("DeliveryLevel") or 1
                local cap=(DeliveryConfig.Levels[lvl] and DeliveryConfig.Levels[lvl].Capacity) or 4000
                for item,cnt in pairs(inv) do if DeliveryConfig.Items[item] and cnt>0 then local w=DeliveryConfig.Items[item].Weight or 1; local can=math.min(cnt, math.floor((cap-totalW)/w)); if can>0 then payload[item]=can; totalW+=can*w end end end
...
task.spawn(function()
    while alive() do
        ...
        if cfg.autoAmbilMagnet and alive() then
            local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
            local farm = workspace.Farm:FindFirstChild("Farm1")
            if hrp and farm then
                local collected=0
                for _, tool in ipairs(farm:GetChildren()) do
                    if not alive() or not cfg.autoAmbilMagnet then break end
                    if tool:IsA("Tool") then
                        local handle = tool:FindFirstChild("Handle") or tool:FindFirstChildWhichIsA("BasePart")
                        local prompt = handle and handle:FindFirstChild("ProductPrompt")
                        local should = cfg.itemDiambil=="Semua" or (cfg.itemDiambil=="Emas & Cosmic" and (tool.Name:find("Emas") or tool.Name:find("Cosmic"))) or (cfg.itemDiambil=="Sakura & Cosmic" and (tool.Name:find("Sakura") or tool.Name:find("Cosmic"))) or (cfg.itemDiambil=="Emas" and tool.Name:find("Emas")) or (cfg.itemDiambil=="Cosmic" and tool.Name:find("Cosmic"))
                        if should and prompt and prompt.Enabled then
                            local dist=(hrp.Position - tool:GetPivot().Position).Magnitude
                            if dist<=40 then
                                pcall(function() hrp.CFrame = handle.CFrame * CFrame.new(0,3,0) end)
                                task.wait(0.3)
                                pcall(function() fireproximityprompt(prompt,0) end)
                ...
                if collected>0 then log("Ambil otomatis "..collected) end
            end
        end
    end
end)
    ...
    while alive() do
        ...
        if cfg.autoIsiAir and pct < cfg.isiSaat and (os.clock()-lastRefill)>10 then
            if LocalPlayer:GetAttribute("IsAutoRefill") ~= true then
                ...
                local well=workspace.Farm:FindFirstChild("Farm1") and workspace.Farm.Farm1:FindFirstChild("ActiveWell")
        ...
        if cfg.autoSiram then
            local hrp=LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
            if hrp then
                local grassData=getGrassData()
                local nearest, bestDist=nil, math.huge
                for _,g in ipairs(grassData) do if g.growth and g.growth<75 then
                    local pos=Vector3.new(g.pos[1], g.pos[2], g.pos[3])
                    if isInFarm1(pos) then
                        local d=(hrp.Position-pos).Magnitude
                        if d<bestDist then bestDist=d; nearest=g end
                    end
                end end
                if nearest then
                    local pos=Vector3.new(nearest.pos[1], nearest.pos[2]+4, nearest.pos[3])
                    if not isInFarm1(pos) then pos=getFarm1RandomPos() end
                    local dist=(hrp.Position-pos).Magnitude
                    if dist>7 then
                        if dist>50 then hrp.CFrame=CFrame.new(pos) else TweenService:Create(hrp, TweenInfo.new(math.clamp(dist/18,0.5,1.2), Enum.EasingStyle.Linear), {CFrame=CFrame.new(pos)}):Play() end
                        log("Menuju rumput ["..math.floor(pos.X)..","..math.floor(pos.Z).."]")
                        task.wait(0.7)
                    end
                else
                    local rnd=getFarm1RandomPos()
                    if (hrp.Position-rnd).Magnitude>10 then TweenService:Create(hrp, TweenInfo.new(1), {CFrame=CFrame.new(rnd)}):Play(); task.wait(0.7) end
                end
            end
            ...
            if tool then
                if tool.Parent~=LocalPlayer.Character then tool.Parent=LocalPlayer.Character; task.wait(0.4) end
                local remote=tool:FindFirstChild("WaterRemote")
                local before=getWaterAmount()
                if remote and before>8 then
                    pcall(function() remote:FireServer("StartPouring") end)
                    task.wait(cfg.waktuSiram)
                    pcall(function() remote:FireServer("StopPouring") end)
                    task.wait(0.8)
